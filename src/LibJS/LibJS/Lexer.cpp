...
namespace SN::LibJS
{
  Lexer::Lexer(Source source) : m_source(source), m_current_char(source[0]), m_hit_invalid_unicode_character(source.length() + 1)
  {
    if (m_keywords.empty())
    {
  ...
  Token Lexer::next()
  {
    ...
    else if (is_valid_identifier_start())
    {
      ...
      auto identifier = m_source.substr(m_value_start - 1, m_position - m_value_start);
    ...
    else if (match('\'') || match('"'))
    {
      ...
      while (!is_eof() && m_current_char != m_string_literal_stop_char && m_current_char != LINE_FEED && m_current_char != CARRIAGE_RETURN)
      {
        if (m_current_char == '\\')
        {
      ...
      auto string_literal = m_source.substr(start_string_literal - 1, m_position - start_string_literal);
    ...
    else
    {
      ...
      {
        if (auto it = m_three_char_tokens.find(m_source.substr(m_position - 1, 3)); it != m_three_char_tokens.end())
        {
          consume();
          consume();
          consume();
      ...
      {
        if (auto it = m_two_char_tokens.find(m_source.substr(m_position - 1, 2)); it != m_two_char_tokens.end())
        {
          consume();
          consume();
          return {it->second, "", ""};
      ...
      if (auto it = m_one_char_tokens.find(m_current_char); it != m_one_char_tokens.end())
      {
        consume();
        return {it->second, "", ""};
      }
  ...
  void Lexer::consume()
  {
    ...
    if (is_line_terminator())
    {
      std::cout << "Line terminator(" << std::dec << m_line_number << ", " << m_column_number << "): "
                << "0x" << std::hex << std::setw(4) << std::setfill('0') << (u32)m_current_char << std::endl;
      m_line_number++;
      m_column_number = 1;
    }
    else if (is_unicode_character())
    {
      std::cout << "Unicode(" << std::dec << m_line_number << ", " << m_column_number << "): "
                << "0x" << std::hex << std::setw(4) << std::setfill('0') << current_code_point() << std::endl;
  ...
  u32 Lexer::current_code_point()
  {
    ...
    if ((m_current_char & 0b00010000) == 0)
    {
      ...
      return (static_cast<u32>(c1_bits) << 12 | static_cast<u32>(c2_bits) << 6 | static_cast<u32>(c3_bits));
    }
    ...
    if ((m_current_char & 0b00001000) == 0)
    {
      ...
      return (static_cast<u32>(c1_bits) << 18 | static_cast<u32>(c2_bits) << 12 | static_cast<u32>(c3_bits) << 6 | static_cast<u32>(c4_bits));
    }
  ...
  {
    if (m_current_char == SPACE || m_current_char == CHARACTER_TABULATION || m_current_char == LINE_TABULATION || m_current_char == FORM_FEED)
    {
      return true;
    }
  ...
  {
    return (m_current_char >= 'a' && m_current_char <= 'z') || (m_current_char >= 'A' && m_current_char <= 'Z');
  }
  ...
  {
    return is_latin_letter() || is_digit() || m_current_char == '$' || (current_code_point() == ZERO_WIDTH_NON_JOINER) || (current_code_point() == ZERO_WIDTH_JOINER);
  }
  ...
  bool Lexer::match(char a, char b, char c, char d)
  {
    ...
    return m_current_char == a && m_source[m_position] == b && m_source[m_position + 1] == c && m_source[m_position + 2] == d;
  }
}